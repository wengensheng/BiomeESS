...
!===========Main============
 program growthana
 ! Updated 02/08/2023 for the new format of output in BiomeE
 ...
 ! Yearly cohort
 filein1  = trim(filepath)//trim(runID)//'_'//'Cohort_yearly.csv'
 filein2  = trim(filepath)//trim(runID)//'_'//'Ecosystem_yearly.csv'
 ...
 ! -------Hourly cohort----------
 filein1  = trim(filepath)//trim(runID)//'_'//'Cohort_hourly.csv'
...
!========================================================================
 ...
 subroutine cohort_yearly(fin_1,fin_2,fout_1,fout_2)
   ...
   !------local vars -----------
   integer, parameter :: Columns = 35 + 210 ! 28
...
!        woody biomass residence time
         DBH = dataarray(9,j,m)
         iLayer=dataarray(4,j,m)
         iPFT  =dataarray(3,j,m)

!        size classes distribution, including total individuals; biomass distribution
         do i=1,bins
             if(dataarray(9,j,m)>DBHbins(i) .and. dataarray(9,j,m)<DBHbins(i+1) ) then
                DBHclasses(i,m) = DBHclasses(i,m) + dataarray(5,j,m)
                BMclasses(i,m)  = BMclasses(i,m)  + dataarray(5,j,m)*         &
                                  sum(dataarray(13:18,j,m))/10000
              endif
         enddo

         ! Basal area with DBH > 0.1 m
         if(dataarray(9,j,m)> 0.0)then
             basalA(m)   = basalA(m)   + 0.25*PI*dataarray(9,j,m)**2 * dataarray(5,j,m)
             BApft(iPFT,m) = BApft(iPFT,m) + 0.25*PI*dataarray(9,j,m)**2 * dataarray(5,j,m)
         endif
         ! mean dDBH of the first layer
         if(dataarray(4,j,m)<2.0)then
             dDBH(iPFT,m) = dDBH(iPFT,m) + dataarray(7,j,m) * dataarray(5,j,m)
