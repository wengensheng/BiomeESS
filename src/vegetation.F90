...
subroutine vegn_photosynthesis(forcing, vegn)
  ...
  real :: f_light(CLmax) ! Fraction of total light over each layer
  ...
  ! Initialize kp (light extinction "strength" per crown layer).
  vegn%kp = 0.0
  do i = 1, ncoh
    cc => vegn%cohorts(i)
  ...
  ! Light fraction (f_light) reaching each layer.
  ! f_light(1) is at canopy top; f_light(i) propagates through the above layers.
  f_light = 0.0
  f_light(1) = 1.0
  do i = 2, nlayers
  ...
  do i = 1, ncoh
    ...
    associate (sp => spdata(cc%species))
      ...
      if (cc%status == LEAF_ON .and. cc%Aleaf > zero_thld) then
        ...
        cana_q   = (esat(Tair) * forcing%RH * mol_h2o) / (p_surf * mol_air)   ! kg/kg
        ...
        ! For drought mortality diagnostics (accumulate potential demand)
        wd1 = wd * mol_h2o * cc%Aleaf * step_seconds
        cc%totDemand = cc%totDemand + wd1
        cc%dailyWdmd = cc%dailyWdmd + wd1

      else
        ! No leaves => zero photosynthesis & transpiration
        cc%An_op   = 0.0
        cc%An_cl   = 0.0
        cc%gpp     = 0.0
        cc%transp  = 0.0
...
subroutine vegn_growth(vegn)
  ...
  do i = 1, vegn%n_cohorts
    ...
    associate (sp => spdata(cc%species))
      ...
      ! --- Allocate carbon to plant pools --------------------------------
  ...
  if (newcohorts >= 1) then   ! build new cohorts for seedlings
     ...
     do i = 1, newcohorts
        ...
        associate (sp => spdata(reproPFTs(i)))
          ...
          vegn%wcl(2) = Max(0.0, vegn%wcl(2)-cc%nindivs*cc%W_sw/(thksl(2)*1000.0))
...
real function mortality_rate(cc) result(mu) ! per year
  ...
  associate ( sp => spdata(cc%species))
    ...
    f_L = (sp%A_un - 1.) * SQRT(Max(0.0, cc%layer-1.0))     ! Layer effects (0~ infinite)
...
real function PlantWaterSupply(cc,step_seconds) result(pws)
  ...
  associate ( sp => spdata(cc%species) )
    ...
    if(do_simple_W_supply)then ! An arbitrary assignment
      ...
      S_stem = 0.0
      do i =1, n_iterations
        psi_stem = log(W_sw/cc%Wmax_s)/sp%CR_Wood
        dpsi     = max(0.0, psi_stem - psi_leaf) ! sp%psi50_WD
        if(dpsi <= 0.02 .or. W_sw <= cc%Wmin_s)exit
...
subroutine Plant_water2psi_exp(cc)
  ...
  associate ( sp => spdata(cc%species))
    if(cc%Wmax_s > 1.0E-4)then
      W_status = MIN(max(1.0E-4,cc%W_sw),cc%Wmax_s)
...
subroutine Plant_water2psi_linear(cc)
  ...
  associate ( sp => spdata(cc%species) )
    !dW_S =  Max(0.0,MIN(1.0,(cc%Wmax_S - cc%W_sw)/(cc%Wmax_S - cc%Wmin_S)))
